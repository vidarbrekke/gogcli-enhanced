...
Setup **automatically** injects a directive into the OpenClaw workspace bootstrap file `TOOLS.md` and syncs the tracked `gog` skill into `skills/gog/SKILL.md` so the agent uses gog-agentic for Google Drive and Docs **before** trying other paths. OpenClaw includes `TOOLS.md` in the system prompt for every run, so no manual instruction is required.

- **What setup does:** Creates or appends to `$workspace_dir/TOOLS.md` a section *"Google Drive and Docs (gog-agentic MCP)"* and syncs the tracked `gog` skill file to `$workspace_dir/skills/gog/SKILL.md`. Together they instruct the agent to call gog-agentic MCP tools directly and **not** to try mcporter-to-CLI, browser automation, or GOG_KEYRING_PASSWORD—so the agent does not waste time and tokens probing those paths. Idempotent: re-running setup refreshes both files.
