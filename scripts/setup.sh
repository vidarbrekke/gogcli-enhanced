...
PY
  ...
  local tools_section_file="$ROOT_DIR/docs/TOOLS-gog-agentic-section.md"
  python3 - "$tools_md" "$tools_section_file" <<'PY'
import sys, os, re
tools_path = sys.argv[1]
section_file = sys.argv[2] if len(sys.argv) > 2 else None
if section_file and os.path.isfile(section_file):
...
PY
    ...
    python3 - "$tools_md_fb" "$tools_section_file" <<'PY'
import sys, os, re
tools_path = sys.argv[1]
section_file = sys.argv[2] if len(sys.argv) > 2 else None
if section_file and os.path.isfile(section_file):
