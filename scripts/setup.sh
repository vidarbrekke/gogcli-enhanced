...
**Main agent — handle Drive/Docs in this session:** If you are the main agent and the user asks to list Drive, search files, create a folder/doc, or any Drive/Docs task, **do it yourself** by calling the gog-agentic tools. Do not delegate that task to a subagent; subagents often do not receive MCP tools, so they will correctly report "tools not available" and the user gets a dead end.

**Subagent — if you don't have drive.*/docs.*:** If you are a subagent and your tool list does not include `drive.listFiles`, `drive.searchFiles`, `docs.create`, or other `drive.*`/`docs.*` tools, say: "I'm a subagent and don't have Google Drive/Docs tools in my list. Ask in the main chat (e.g. 'List my Google Drive root' or 'Use drive.listFiles to list my Drive') so the main agent can use gog-agentic." Do not claim the gateway is misconfigured; the issue is that subagents get a reduced tool set (runbook §8.9).
