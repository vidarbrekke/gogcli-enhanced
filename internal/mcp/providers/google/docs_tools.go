	...
	return []server.ToolSpec{
		{
			...
			Description: "Create a new Google Doc and optionally apply a batchUpdate in one call.",
			Tier:        "ga",
			Version:     "v1",
			PolicyClass: "write-safe",
			InputSchema: map[string]any{
				...
				"properties": map[string]any{
					"title":    map[string]any{"type": "string"},
					"parentId": map[string]any{"type": "string"},
					"request":  map[string]any{"type": "object", "description": "Optional batchUpdate body with Docs requests"},
				},
			},
			...
			Name:        "docs_mergeData",
			Description: "Generate Docs from a template with mail-merge input.",
			Tier:        "ga",
			Version:     "v1",
			PolicyClass: "write-safe",
			InputSchema: map[string]any{
			...
			Name:        "docs_cat",
			Description: "Extract plain text from a Google Doc with optional maxBytes and tab/allTabs.",
			Tier:        "ga",
			Version:     "v1",
			PolicyClass: "read-fast",
			InputSchema: map[string]any{
			...
			Description: "Return the append index for a Google Doc for inserting new content.",
			Tier:        "ga",
			Version:     "v1",
			PolicyClass: "read-fast",
			InputSchema: map[string]any{
			...
			Description: "Search in a Google Doc and return text match startIndex/endIndex (UTF-16).",
			Tier:        "ga",
			Version:     "v1",
			PolicyClass: "read-fast",
			InputSchema: map[string]any{
			...
			Name:        "docs_export",
			Description: "Export a Google Doc to pdf, docx, or txt; if out is omitted, returns a temp file path.",
			Tier:        "ga",
			Version:     "v1",
			PolicyClass: "read-fast",
			InputSchema: map[string]any{
